...
#define ARG_PUSH_DEBUG_ONLY(v, arg)
    ...
    ARG_PUSH_FMT(argv, "-Dporter.library.path=%s", lib_path)
...
int main(int argc, char *argv[]) {
    ...
    uid_t uid = getuid();
    if (uid != 0 && uid != 2000) {
        perrorf("fatal: run Porter from non root nor adb user (uid=%d).\n", uid);
        exit(EXIT_FATAL_UID);
    }
    ...
    if (replace_pid != 0) {
        ...
        foreach_proc([](pid_t pid) {
            ...
            if (kill(pid, SIGKILL) == 0)
                ...
                perrorf("fatal: can't kill %d, please try to stop existing Porter from app first.\n", pid);
