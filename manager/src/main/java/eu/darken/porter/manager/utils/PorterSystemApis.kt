...
class PorterSystemApis internal constructor(
     ...
     */
    private fun checkReady() = check(ready) { "PorterSystemApis used before attachToSystemServices()" }

    companion object {
        @Volatile
        private var _instance: PorterSystemApis? = null

        val instance: PorterSystemApis
            get() = _instance ?: synchronized(this) {
                _instance ?: PorterSystemApis().also { _instance = it }
            }
