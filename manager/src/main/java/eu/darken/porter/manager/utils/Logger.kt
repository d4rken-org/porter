...
class Logger(private val tag: String) {
    ...
    fun v(fmt: String, vararg args: Any?) {
        if (isLoggable(tag, Log.VERBOSE)) Log.v(tag, String.format(Locale.ENGLISH, fmt, *args))
    }
    ...
    fun d(fmt: String, vararg args: Any?) {
        if (isLoggable(tag, Log.DEBUG)) Log.d(tag, String.format(Locale.ENGLISH, fmt, *args))
    }
