...
直接支持 Porter 的应用只需安装 Porter。若某个应用仅支持 Shizuku，请在 FOSS 版本中打开 **Shizuku 兼容性**，安装内置的兼容应用，并在卸载 Shizuku 前查看替换与导入的内容。同一发布版本中独立的 **Porter Compatibility** APK 同样可用。手动安装时请先卸载 Shizuku，因为兼容应用无法与它同时安装。使用这些应用期间，请保留兼容应用。详情见[兼容性指南](/compatibility)。

## 选择启动方式
{: #choose-how-to-start }

服务停止时，启动方式会直接显示在主屏幕上。
...
在支持无线调试的电视上，请在 Porter 中选择**配对**，并按提示启用其配对无障碍服务。请在一分钟内依次打开**开发者选项**、**无线调试**和**使用配对码配对设备**，并在 Porter 读取配对码并完成配对期间保持该对话框打开。

Porter 会在配对码界面上方显示进度面板。请保持该界面打开，直至出现配对结果。随后 Porter 会打开结果界面，并关闭其配对无障碍服务。结果界面会单独确认该服务已关闭，这与配对是否成功无关。成功后请选择**启动**；失败或超时后请选择**重试**。如果 Porter 没有自动打开，请手动打开以查看结果。若安装界面的按钮无法用遥控器选中，请参阅[电视安装警告](/troubleshooting#a-tv-installation-warning-cannot-be-selected-with-the-remote)。
