...
FOSS 版には対応する互換 APK が含まれています。先に Porter をインストールして起動し、ホーム画面または設定から Shizuku 互換設定を開きます。置き換えの内容を確認して確定してください。Porter が Shizuku を停止し、確認できたアクセス許可を自動的に引き継ぎます。Porter がサービスを停止できない場合は、Shizuku でサービスを停止して再試行するよう案内します。Shizuku が未インストールなら、互換機能を直接インストールできます。手動でインストールする場合：
...
組み込みの設定では、確認できたアクセス許可の判断を自動的にインポートします。Porter の既存の判断は保持されます。Shizuku のアプリ設定やペアリングは移行しません。アンインストール前に置き換えの確認画面を確認してください。インストールが失敗しても、アクセス許可の判断は再試行のために保存されます。
